...
def library():
    ...
    return render_template(
        "library.html", year=year, images=images, hexes=hexes
    )
...
def create():
    ...
    if form.validate_on_submit():
        ...
        jumps = round(len(common_hex) / 4)
        print(jumps)
        common_hex = [hex[0] for hex in common_hex[::jumps]]
...
def download(hex):
    ...
    return send_from_directory(
        "static/palettes_text", f"{hex}.txt", as_attachment=True
    )
