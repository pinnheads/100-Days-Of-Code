...
def home():
    ...
    return render_template(
        "Index.html", basketitems=basket_items, products=products
    )
...
def login():
    ...
    if loginform.validate_on_submit():
        user = User.query.filter_by(
            emailaddress=loginform.emailaddress.data
        ).first()
        if not user:
            flash(
                "No user exists with this email address. Maybe you need to register first?"
            )
    ...
    return render_template(
        "Login.html", basketitems=basket_items, form=loginform
    )
...
def register():
    ...
    if registerform.validate_on_submit():
        ...
        if not user:
            ...
            login_user(NewUser)
            flash(
                f"Your account has been created {current_user.name}. Now please login."
            )
            return redirect(url_for("login"))
        else:
            flash(
                "An account already exists for that email address, please try again."
            )
            return redirect(url_for("register"))
    else:
        return render_template(
            "register.html", basketitems=basket_items, form=registerform
        )
    ...
    if "cart" in session:
        ...
        for item in session["cart"]:
            ...
            if dict_of_items[product.id]["qty"] > product.stock_volume:
                ...
                dict_of_items[product.id]["qty"] = product.stock_volume
                flash(
                    "Not enough items in stock. Basket items adjusted to maximum available!"
                )
        session["cart"] = []
